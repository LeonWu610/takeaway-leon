...
export const SWIPE_KEY = Symbol(name) //每次调用都会返回一个不同的 Symbol 值
...
export default defineComponent({ // 创建组件
  name,
  props: {
    autoplay: {
      type: Number,
  ...
  setup(props, { slots }) {
    ...
    const minOffset = computed(() => {   //轨道的最小偏移量
      if (state.rect) {
    ...
    const activeIndicator = computed(() => {   //当前活动指示器的索引
